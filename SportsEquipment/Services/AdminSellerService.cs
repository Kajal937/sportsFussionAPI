...
namespace SportsEquipment.Services
{
    public class AdminSellerService : IAdminSellerService
    {
        ...
        public async Task AddSellerAsync(AdminSeller adminSeller, string productName)
        {
            using var connection = CreateConnection();
            ...
            try
            {
                ...
                var productId = await connection.ExecuteScalarAsync<int?>(getProductSql, new { product_name = productName }, transaction);
